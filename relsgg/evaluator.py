"""Evaluators for relation prediction tasks.

Currently implemented:
    SGClsEvaluator     — strict R@K and mR@K for scene graph classification
                         (oracle-box / SGCls protocol, Xu et al. 2017).
    SoftSGClsEvaluator — synonym-aware SoftR@K / SoftmR@K: a prediction
                         matches GT if the predicates share a canonical form
                         or their text embeddings' cosine ≥ tau_eval. With a
                         10K synonym-preserving vocabulary, strict matching
                         counts correct synonyms as misses — this is the
                         primary MEGASG-val metric (plan §9a).
"""
...
class KeySwapEvaluator:
    """Feed an inner evaluator from an alternate logits key.

    Used to score the fast bilinear head (``out["logits_fast"]``) in the
    same eval pass as the main head; its metrics are prefixed so both appear
    side by side. Batches without the key are skipped (e.g. train-time)."""

    def __init__(self, inner, logits_key: str = "logits_fast",
                 prefix: str = "fast_") -> None:
        self.inner = inner
        self.logits_key = logits_key
        self.prefix = prefix

    def update(self, out: dict, targets: List[dict]) -> None:
        if self.logits_key in out:
            self.inner.update({**out, "logits": out[self.logits_key]}, targets)

    def compute(self, **kw) -> Dict[str, float]:
        try:
            m = self.inner.compute(**kw)
        except TypeError:
            m = self.inner.compute()
        return {self.prefix + k: v for k, v in m.items()}
...
def frequency_metrics(tp: Dict[int, int], gt: Dict[int, int],
    ...
    """Long-tail-aware aggregates that plain R@K and mR@K both miss.

    Motivation, measured on v37 vs v35 (VG150 test): R@50 read +120% while
    mR@50 read -28%, because the model had collapsed onto head predicates.
    Neither number alone reveals that; the bucket split does (rare -100%,
    common -60%, frequent -6%).
    ...
    per-image recalls, so each class contributes once at its own support:
      ...
      wR@K                    IDF-weighted recall, w_c ∝ log(N/n_c)
                              ...
                              instead of by hard buckets.

    NOTE ON VARIANCE — tail weight COSTS variance, monotonically. Measured
    SE(@50) with v35's per-class recalls:

        split          micro R   macro mR   IDF wR
        vg150/test       0.09p     0.48p     0.82p
        psg/test         0.35p     0.47p     0.56p
        psg/val          0.48p     1.13p     1.34p

    So wR is the NOISIEST of the three, not the most stable. Prefer the
    bucket split for reporting (it is LVIS-standard and decomposed, so head
    collapse cannot hide in it) and macro mR for model selection. wR is a
    supporting scalar, not a headline.
    """
    out: Dict[str, float] = {}
