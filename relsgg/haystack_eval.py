...
from relsgg.evaluator import FREQ_BUCKETS
...
class HaystackEvaluator:
    ...
    def __init__(self, neg_by_index: Dict[int, List[List[int]]],
        ...
        # fAP is a CROSS-PAIR per-predicate metric, so — exactly as on
        # SpatialSense — it is dominated by `pair_logits`, the only cross-pair
        # term. That term is a contact/annotation-propensity prior
        # ([[relsgg-relatedness-contact-prior]]), and its WEIGHT relative to the
        # predicate logits is set by logit_scale, which was frozen at init until
        # v44 ([[relsgg-untrained-output-head]]). Dropping it here is the
        # control that says whether v44's +17.3% HICO fAP was a representation
        # gain or the same rescaling effect.
