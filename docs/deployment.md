...
The detector and relation head overlap on a worker thread: **28.5 FPS** for
YOLOE + RelSGG. CUDA streams and static-shape specialization gave zero
additional gain; batch-size-1 inference is CPU-dispatch bound (~1362 kernels),
so the win is overlap, not kernel efficiency.
...
<https://maelic.github.io/RelateAnything_demo/>. Model files are regenerated by
